...
import { sendSlackTest } from '@/lib/modules/integrations/actions'
...
export function TestSlackButton() {
  ...
  function onClick() {
    ...
    startTransition(async () => {
      ...
      if (!r.configured) setMsg({ tone: 'warn', text: 'Slack not configured. Set SLACK_WEBHOOK_URL in your env.' })
  ...
  return (
    <div className="space-y-2">
      <Button type="button" variant="outline" size="sm" onClick={onClick} disabled={pending}>
        {pending ? 'Sending…' : 'Send Slack test'}
      </Button>
